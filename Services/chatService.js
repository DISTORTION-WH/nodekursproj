...
async function deleteMessagesByChat(chatId) {
  return client.query(`DELETE FROM messages WHERE chat_id = $1`, [chatId]);
}

// Удалить связи пользователей с чатом
async function deleteChatUsers(chatId) {
  return client.query(`DELETE FROM chat_users WHERE chat_id = $1`, [chatId]);
}

// Удалить сам чат
async function deleteChat(chatId) {
  return client.query(`DELETE FROM chats WHERE id = $1`, [chatId]);
}
