...
export default function AdminPage() {
  ...
  // === Чаты ===
  useEffect(() => {
    axios.get("http://localhost:5000/admin/chats", { headers: authHeaders })
      .then(res => {
  ...
  const handleDeleteUser = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/admin/users/${id}`, { headers: authHeaders });
  ...
  const handleSaveUser = async () => {
    try {
      await axios.put(`http://localhost:5000/admin/users/${editingUser.id}`, editingUser, { headers: authHeaders });
  ...
  const handleDeleteChat = async (chat) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/admin/chats/${chat.id}`, { headers: authHeaders });
