...
    if (!token) return;
    axios
//       .get("http://localhost:5000/friends", { headers: authHeaders }) // 👈 БЫЛО
      .get("/friends", { headers: authHeaders }) // 👈 СТАЛО
...
    try {
      await axios.put(
//         "http://localhost:5000/users/password", // 👈 БЫЛО
        "/users/password", // 👈 СТАЛО
...
          </li>
        ))
      : [<div key="no-friends" className="profile-no-friends">Нет друзей</div>];
...
            <p><strong>Зарегистрирован: </strong>{currentUser?.created_at ? new Date(currentUser.created_at).toLocaleString() : "—"}</p>
...
                <button className="profile-upload-btn" onClick={handleUploadClick}>Сменить аватар</button>
...
            </button>
            <button className="profile-logout-btn" onClick={handleLogout}>Выйти</button>
